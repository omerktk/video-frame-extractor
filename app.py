...
def list_videos(folder):
    return [f for f in os.listdir(folder) if os.path.isfile(os.path.join(folder, f)) and f.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.dat'))]

def list_folders(folder):
    return [f for f in os.listdir(folder) if os.path.isdir(os.path.join(folder, f))]

def video_to_images(video_path, output_folder, fps, upscale_factor=1, interpolation=cv2.INTER_CUBIC):
    ...
    while True:
        ...
        if frame_number % frame_interval == 0:
            if upscale_factor != 1:
                width = int(frame.shape[1] * upscale_factor)
                height = int(frame.shape[0] * upscale_factor)
                dim = (width, height)
                frame = cv2.resize(frame, dim, interpolation=interpolation)
...
def process_dat_file(dat_path, output_folder, upscale_factor=1, interpolation=cv2.INTER_CUBIC):
    video_to_images(dat_path, output_folder, 1, upscale_factor, interpolation)
...
def index():
    if request.method == 'POST':
        ...
        if 'file' in request.files:
            ...
            elif file and file.filename.lower().endswith(('.mp4', '.avi', '.mov', '.mkv', '.dat')):
        ...
        if 'fps' in request.form and 'upscale_factor' in request.form:
            fps = int(request.form.get('fps', 1))
            upscale_factor = float(request.form.get('upscale_factor', 1))
            ...
            for video in selected_videos:
                ...
                if video.lower().endswith('.dat'):
                    process_dat_file(video_path, video_output_folder, upscale_factor)
                else:
                    video_to_images(video_path, video_output_folder, fps, upscale_factor)

            flash(f'Processed {len(selected_videos)} videos at {fps} frames per second with upscale factor {upscale_factor}')
